    ...
    if hasattr(value, 'item'):
        try:
            ...
            if cleaned is None or (
                isinstance(cleaned, float) and
                (math.isnan(cleaned) or math.isinf(cleaned))
            ):
                return None
            return cleaned
        except Exception:
...
def clean_dataframe_for_json(df: pd.DataFrame) -> list:
    ...
    return [
        {key: clean_value(val) for key, val in record.items()}
...
# HELPERS
# ─────────────────────────────────────────────────────────────
def _require_data(endpoint: str = "") -> dict:
    """
    Shared guard: raises 400 if no real data has been uploaded yet.
    Returns schema dict if data exists.
    """
    schema = get_table_schema("uploaded_data")

    no_columns = not schema.get("columns")
    no_rows    = schema.get("row_count", 0) == 0

    if no_columns or no_rows:
        print(f"[{endpoint}] Blocked — no data uploaded yet")
        raise HTTPException(
            status_code=400,
...
async def get_schema():
    """Get database schema information (columns, stats, sample data)."""
    print(f"\n{'='*60}")
    ...
    try:
        ...
        # ── No columns = skeleton table = nothing uploaded ─────────────────
        ...
        # ── Columns exist but zero rows = empty table ──────────────────────
...
async def ask_question(request: AskRequest):
    ...
    try:
        ...
        if not execution_result["success"]:
            print(f"[ASK] Execution failed: "
                  f"{execution_result.get('error', 'Unknown error')}")
            raise HTTPException(
                status_code=400,
                detail=f"SQL Execution failed: "
                       f"{execution_result.get('error', 'Unknown error')}"
            )
        ...
        print(f"[CACHE] Stored (TTL: {QUERY_CACHE.ttl}s)")
...
async def download_csv():
    ...
    try:
        ...
        print(f"[DOWNLOAD] CSV ready — {len(df)} rows")
...
async def download_excel():
    ...
    try:
        ...
        print(f"[DOWNLOAD] Excel ready — {len(df)} rows")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Excel download failed: {str(e)}"
        )
